...
def save_images(
    pred_dict, output_dir, num_classes, colours, extra_identifier=""
):
    for id in pred_dict:
       save_image(pred_dict[id].unsqueeze(0).cpu().numpy(), output_dir, num_classes, colours, extra_identifier=extra_identifier)
...
def save_image(image_numpy_array, output_dir, num_classes, colours, extra_identifier=""):
    ...
    im = pipe(
        (im_array * 255).astype(np.uint8).squeeze(),
        _chw_to_hwc,
        Image.fromarray,
    )

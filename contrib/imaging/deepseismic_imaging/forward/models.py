...
class Model(object):
    def __init__(
        ...
        self.grid = Grid(shape=shape_pml, extent=extent_pml, origin=origin_pml, dtype=dtype, subdomains=subdomains,)
        self.n_pml = n_pml
        self.pml = Function(name="pml", grid=self.grid)
        pml_data = np.pad(np.zeros(shape, dtype=dtype), [(n_pml,) * 2 for _ in range(self.pml.ndim)], mode="edge",)
        ...
        for d in range(self.pml.ndim):
            for i in range(n_pml):
                ...
                pml_data[tuple(idx)] += val / self.grid.spacing[d]
        pml_data = np.pad(pml_data, [(i.left, i.right) for i in self.pml._size_halo], mode="edge",)
...
class VelocityModel(Model):
    ...
    def vp(self, vp: Union[float, np.ndarray]) -> None:
        self._vp = vp
        if isinstance(vp, np.ndarray):
            pad_widths = [(self.n_pml + i.left, self.n_pml + i.right) for i in self.m._size_halo]
            self.m.data_with_halo[:] = np.pad(1.0 / self.vp ** 2.0, pad_widths, mode="edge")
        else:
    ...
    def solve(
        ...
        u = TimeFunction(name="u", grid=self.grid, time_order=2, space_order=space_order)
        H = u.laplace
        ...
        src_term = source.inject(field=u.forward, expr=source * self.time_spacing ** 2 / self.m)
